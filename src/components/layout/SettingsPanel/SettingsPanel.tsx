...
import { ACCENT_PRESETS, getSkinSwatches } from '@/lib/theme.config';
...
/** Orden de la galeria de skins. Los textos salen de i18n; aca solo vive
 *  la key de la skin y sus dos claves de traduccion. */
const SKIN_ORDER: {
  key: SkinKey;
  ...
  {
    key: 'default',
    nameKey: 'settings.skinDefault',
    descKey: 'settings.skinDefaultDesc',
  },
  {
    key: 'premium',
    nameKey: 'settings.skinPremium',
    descKey: 'settings.skinPremiumDesc',
  },
  {
    key: 'sakura',
    nameKey: 'settings.skinSakura',
    descKey: 'settings.skinSakuraDesc',
  },
  {
    key: 'midnight',
    nameKey: 'settings.skinMidnight',
    descKey: 'settings.skinMidnightDesc',
  },
  {
    key: 'journal',
    nameKey: 'settings.skinJournal',
    descKey: 'settings.skinJournalDesc',
  },
  {
    key: 'neon',
    nameKey: 'settings.skinNeon',
    descKey: 'settings.skinNeonDesc',
  },
];
...
export function SettingsPanel({ open, onClose }: SettingsPanelProps) {
  ...
  const accentIsAuto = !customAccent && accent === null;
    ...
    >
      ...
      <section className="settings-panel__section">
        ...
        <div className="settings-panel__field">
          ...
          <p className="settings-panel__hint">{t('settings.skinHint')}</p>
          <div
            className="settings-panel__skins-grid"
            role="radiogroup"
            aria-label={t('settings.skinLabel')}
          >
            {SKIN_ORDER.map((opt) => {
              const isActive = skin === opt.key;
              const swatches = getSkinSwatches(opt.key, theme);
              return (
                <button
                  key={opt.key}
                  type="button"
                ...
                >
                  <div className="settings-panel__skin-header">
                    <span className="settings-panel__skin-title">
                      {t(opt.nameKey)}
                    </span>
                  ...
                  <div className="settings-panel__skin-desc">
                    {t(opt.descKey)}
                  </div>
                </button>
              );
          ...
          <div className="settings-panel__accent-row">
            ...
            >
              <button
                type="button"
                role="radio"
                aria-checked={accentIsAuto}
                aria-label={t('settings.accentAuto')}
                title={t('settings.accentAuto')}
                onClick={() => setAccent(null)}
                className={`settings-panel__accent settings-panel__accent--auto${
                  accentIsAuto ? ' settings-panel__accent--active' : ''
                }`}
              >
                A
              </button>
            ...
            <ColorPicker
              value={
                customAccent ??
                (accent ? ACCENT_PRESETS[accent].swatch : undefined)
              }
      ...
      {/* ── Preferencias comunitarias y spoilers ── */}
      <section className="settings-panel__section">
        <h3 className="settings-panel__section-title">
